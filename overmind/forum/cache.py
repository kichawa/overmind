...
def cache_view(key, groups=(), last_modified_func=None):
    ...
    def decorator(view):
        ...
        def wrapper(request, *args, **kwargs):
            ...
            # cache both `last_modified` and `view` function call results
            # using single key
            # {
            #     'last_modified': <view last modifield date>,
            #     'response': <response generated by view>,
            # }
            cached_view = cache.getset(key, {})

            if last_modified_func:
                def cached_last_modified_func(*args, **kwargs):
                    last_modified = cached_view.get('last_modified', nil)
                    if last_modified is nil:
                        last_modified = last_modified_func(*args, **kwargs)
                        cached_view['last_modified'] = last_modified
                        cache.set(key, cached_view,
                                  groups=gbuilder(request.GET, kwargs))
                    return last_modified
                view_fn = condition(last_modified_func=cached_last_modified_func)(view_fn)

            response = cached_view.get('response', nil)
            if response is nil:
                response = view_fn(request, *args, **kwargs)
                if response.status_code in CACHABLE_RESPONSE_CODE:
                    cached_view['response'] = response
                    cache.set(key, cached_view,
                              groups=gbuilder(request.GET, kwargs))
            return response
